			...
			<div class="breadcrumb">
				...
				<div class="path">
					<span>Dashboard</span> • <span>Product</span> • <span
						class="current">Edit Product</span>
				</div>
			</div>
			<c:if test="${not empty error}">
				<p style="color: red;">${error}</p>
			</c:if>
			<form action="${contextPath}/admin/products/edit"
				...
				<div class="form-section">
					...
					<textarea name="fullDescription"
						placeholder="Write something awesome about the product...">${product.fullDescription}</textarea>
				</div>
				<div class="form-section">
					<h5>Images</h5>
							...
							<select id="category" name="category" required>
								...
								<c:forEach var="category" items="${categories}">
									<option value="${category.name}"
										${category.name == product.category ? 'selected' : ''}>${category.name}</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							<select id="colors" name="colors">
								...
								<option value="Natural"
									${product.color == 'Natural' ? 'selected' : ''}>Natural</option>
								<option value="Orange"
									${product.color == 'Orange' ? 'selected' : ''}>Orange</option>
								<option value="White"
						...
						<div class="form-group discounted-price-group">
							<div class="discount-toggle">
								<input type="checkbox" id="enableDiscount"
									...
									${product.discountedPrice > 0 ? 'checked' : ''} /> <input
									type="number" id="discountedPrice" name="discountedPrice"
									placeholder="Enter discounted price" step="0.01" min="0"
									value="${product.discountedPrice > 0 ? product.discountedPrice : ''}"
									${product.discountedPrice > 0 ? '' : 'disabled'} />
							</div>
						</div>
					</div>
				</div>
				<label> <input type="checkbox" name="publish"
					${product.published ? 'checked' : ''} /> Publish
				</label>
